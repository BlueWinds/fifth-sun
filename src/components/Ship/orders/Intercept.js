...
import {progressAllOrbits, getFullCartesian, maxOrbitalSpeed, betweenPoints} from '@lib/orbits'
import travelTime from '@lib/travelTime'
import displayVelocity from '@lib/displayVelocity'

export default function Intercept({order, path, update, duration}) {
  const label = <>
    Intercept <SelectObject currentTarget={order.target} onSelect={target => update({...order, target})} />
  </>

  return <Order id={order.id} label={label} path={path} duration={duration}></Order>
}

export const label = 'Intercept'
